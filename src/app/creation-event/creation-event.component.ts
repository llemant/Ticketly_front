...
export class CreationEventComponent implements OnInit {
  ...
  constructor(
    public creationEvent: CreateeventService,
  ...
  creerEvent(event: any) {
    ...
    if (!this.regexPhoto.test(event.image)) {
      this.creationEvent.msgErr = this.msgPhotoIncorrect;
    }

    if (
      this.regexGenre.test(event.genre) &&
      this.regexPrix.test(event.prix) &&
      this.regexPhoto.test(event.image)
    ) {
      if (this.authService.getUserSession().nbToken < 200) {
        this.creationEvent.msgErr =
          "Merci d'acheter des tokens pour créer votre événement, la création d'un événement vous coutera 200 tokens";
      } else {
