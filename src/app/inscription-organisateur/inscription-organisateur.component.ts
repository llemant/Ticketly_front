...
export class InscriptionOrganisateurComponent implements OnInit {
  ...
  msgSiretIncorrect =
    'Le format du SIRET est incorrect : il doit contenir 14 chiffres';
  ...
  inscriptionOrganisateur(val: any) {
    ...
    if (
      ...
      this.regexSiret.test(this.user.siret)
    ) {
      this.http
        .get(this.host.myDevHost + 'user/login/' + this.user.login)
        .subscribe({
          next: (data) => {
            ...
            if (this.existingLogin != null) {
              ...
              this.http
                .get(this.host.myDevHost + 'user/email/' + this.user.email)
                .subscribe({
                  next: (data) => {
                    ...
                    if (this.existingEmail != null) {
                      ...
                      this.http
                        .post(this.host.myDevHost + 'organisateur', val)
                        .subscribe({
                          next: (data) => {
